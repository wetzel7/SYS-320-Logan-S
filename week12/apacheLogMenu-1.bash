...
function displayOnlyPages(){
	...
	cat "$logFile" | cut -d ' ' -f 7 | sort | uniq -c | sort -rn
}
...
function histogram(){
	...
	# echo "$visitsPerDay"
        ...
        :> /tmp/newtemp.txt  # what :> does is in slides
	...
	do
		...
		echo "$IP $withoutHours" >> /tmp/newtemp.txt
	done 
	cat "/tmp/newtemp.txt" | sort -n | uniq -c
}
...
do
	...
	echo "[4] Histogram"
	# Frequent visitors
	echo "[5] Display frequent visitors (10+ visits)"
	# Suspicious visitors
